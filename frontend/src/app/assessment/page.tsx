      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="flex items-center gap-3 mb-6 md:mb-8 animate-fade-in">
          ...
          <div className="min-w-0">
            <h1 className="text-xl md:text-2xl font-bold">Avaliação Diagnóstica</h1>
            <p className="text-xs md:text-sm text-muted-foreground">Vamos avaliar seu conhecimento em Álgebra Linear</p>
          </div>
        </div>

        {/* Progress Bar */}
        <div className="glass-card p-3 md:p-4 mb-4 md:mb-6 animate-fade-in">
        ...
        {currentQuestion && (
          <div className="glass-card p-4 md:p-6 lg:p-8 animate-slide-up">
            ...
            {/* Question text */}
            <h2 className="text-base md:text-lg lg:text-xl font-semibold mb-4 md:mb-6 leading-relaxed">
              ...
              {currentQuestion.options.map((option, i) => {
                ...
                return (
                  <button
                    ...
                    disabled={!!feedback}
                    className={`w-full text-left p-3 md:p-4 rounded-xl border-2 transition-all duration-200 flex items-start gap-2 md:gap-3
                      ${showCorrect
                    ...
                    `}
                  >
                    <span className={`w-7 h-7 md:w-8 md:h-8 rounded-lg flex items-center justify-center text-xs md:text-sm font-bold shrink-0
                      ${showCorrect ? 'bg-success/20' : showWrong ? 'bg-destructive/20' : isSelected ? 'bg-primary/20' : 'bg-muted'}`}>
                      {showCorrect ? <CheckCircle2 className="w-4 h-4 md:w-5 md:h-5" /> : showWrong ? <XCircle className="w-4 h-4 md:w-5 md:h-5" /> : letter}
                    </span>
                    <span className="pt-0.5 md:pt-1 text-sm md:text-base">{option}</span>
                  </button>
                );
              })}
            </div>
            ...
            {/* Feedback message */}
            {feedback && (
              <div className={`p-3 md:p-4 rounded-xl mb-3 md:mb-4 animate-fade-in ${feedback.isCorrect ? 'bg-success/10 border border-success/20' : 'bg-destructive/10 border border-destructive/20'}`}>
